    ...
    let completePromise = new Promise((resolve, reject) => {
      ...
      macNetworkDrive.list().then(networkDrives => {
        for (let currentDrivePath in networkDrives) {
          ...
          if (drivePath.charAt(drivePath.length - 1) != "\\") drivePath += "\\";
        ...
        if (result !== undefined) {
          ...
          let mountScript = `
            ...
            with timeout of 2 seconds
              try
                mount volume "${pathDrive}" as user name "${username}" with password "${password}"
              end try
              end timeout
            end tell`;
          ...
          applescript.execString(mountScript, (err, result) => {
            if (err || result === undefined) {
              if (err === undefined) reject("Unable to connect");
